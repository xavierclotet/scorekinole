...
<main class="landing" data-theme={$adminTheme}>
	...
	<div class="content">
		...
		<!-- Quick Links Section -->
		<div class="links-section flex flex-col gap-2 max-w-[400px] w-full">
			...
			<button class="link-card stats-link flex items-center justify-center gap-2 h-12 px-4 bg-link-card-bg border border-link-card-border rounded-lg text-link-card-text text-[0.85rem] font-medium cursor-pointer transition-all hover:bg-link-card-bg-hover hover:border-link-card-border-hover hover:text-link-card-text-hover" onclick={goToMyStats}>
				<BarChart3 class="w-[18px] h-[18px] shrink-0" />
				<span>{m.stats_myStatistics()}</span>
			</button>
		</div>
