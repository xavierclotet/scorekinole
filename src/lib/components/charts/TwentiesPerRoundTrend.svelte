		...
		const chart = new Chart(canvas, {
			...
			data: {
				...
				datasets: [{
					...
					backgroundColor: perRoundData.averages.map((_, i) => {
						const opacity = 0.5 + (i / Math.max(perRoundData.averages.length - 1, 1)) * 0.5;
					...
					tooltip: {
						...
						callbacks: {
							label(ctx: any) {
								...
								return ` ${m.stats_avgTwenties()}: ${perRoundData.averages[idx]} (${perRoundData.counts[idx]} ${m.stats_rounds()})`;
							},
						},
					},
				},
