...
// SPA shell (adapter-static fallback) used for offline navigation to any
// non-prerendered route. The prerendered landing owns / (index.html).
const FALLBACK_SHELL = '/200.html';
...
// Install: cache build + essential static assets + prerendered pages + SPA shell
	...
	event.waitUntil(
		caches
			...
			.then(async (cache) => {
				...
				await cache.addAll([...prerendered, FALLBACK_SHELL]);
			})
			.then(() => sw.skipWaiting())
	);
});
...
sw.addEventListener('fetch', (event) => {
	...
	// For HTML navigation: network-first with offline fallback — prerendered
	// page if we have it (e.g., /), otherwise the SPA shell
	if (event.request.mode === 'navigate') {
		event.respondWith(
			fetch(event.request).catch(async () => {
				const cached = await caches.match(event.request, { ignoreSearch: true });
				return cached ?? (caches.match(FALLBACK_SHELL) as Promise<Response>);
			})
		);
		return;
	}
