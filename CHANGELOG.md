...
- Tests: 4 new ranking cases covering player-nationality filter, CAT→ES folding, foreign-player and guest exclusion; full ranking suite 50/50 passing

## [2.5.38] - 2026-05-06
- Fix (tournament registration, public detail page): close 8 audited bugs in self-registration. (1) `registerForTournament` now passes the proposed `partnerData.userId` into `validateRegistration` inside the transaction, closing the race where two users could both pick the same REGISTERED partner. (2) Auto-promotion from waitlist now re-validates partner eligibility via the new `sanitizePromotedPartner` helper — stale REGISTERED refs that became primary or another team's partner are dropped instead of producing duplicates. (3) `unregisterFromTournament` handles the user-only-as-partner case via the new `detachUserFromPartnerSlots` helper — clicking "Desapuntarme" while listed only as a partner now succeeds (was returning `Not registered`). (4) `WaitlistEntry` gains optional `email` / `photoURL` so they survive the waitlist round-trip and the promoted participant keeps their avatar/email. (5) Doubles-aware capacity labels: stats row, capacity legend and enrolled list now show "Parejas" / "X parejas disponibles" via new Paraglide keys `registration_pairs` and `registration_pairsSpotsLeft`. (6) Explicit `isWaitlistAllowed` helper replaces inline `=== false` / `!== false` drift. (7) New `requestSequencer` util drops stale partner-search responses (typeahead race fix). (8) `partnerSearchResults` is now `$derived.by` so externally-registered or just-selected users disappear from the dropdown live without a new search
- Hardening: reject duplicate GUEST partner names (case + whitespace tolerant) via new `collectGuestPartnerNames` helper; defense-in-depth email-verification gate at registration via new `isCurrentEmailVerified` helper
- Tests: 41 new unit tests across `tournamentRegistration.test.ts`, `auth.test.ts`, and `requestSequencer.test.ts`. Full suite 2136/2136 passing
...
- Known limitations (intentional): (a) cannot undo across game boundaries — `currentGameRounds` is reset per game; (b) cannot undo a round that already triggered match completion — blocked by the existing `isMatchComplete || hasWon` guard at the top of `decrementScore`; (c) the popped round's 20s are not restored to `team.twenty` (re-enter from 0 if you want to re-play that round); (d) rapid cascading undos can race because `handleRoundUndo` is async but fired synchronously — last write wins, no serialisation. All of these were judged acceptable for v1; mitigations can be added on demand

## [2.5.34] - 2026-05-04
- Feat (friendly mode, Phase 1): swipe-down on the score now undoes the last completed round when scores match `lastRoundPoints` (the boundary). Atomic local revert via new `undoLastRound()` in `matchState.ts`: pops the round from `currentGameRounds` / `currentMatchRounds` / `matchState` / `currentMatch.rounds`, decrements `roundsPlayed`, subtracts the popped round's points from `lastRoundPoints`, and persists to localStorage. Caller in `TeamCard.svelte` uses the returned round to revert team-level state — accumulated `points`, `rounds`-won counter for whoever won that round, and the hammer rotation (restored to `popped.hammerTeam`)
- Tournament mode is intentionally gated (`if (inTournamentMode) return`) — Phase 2 will wire up `runTransaction`-based Firestore revert so admin / public view / other devices stay in sync
- New helper `removeLastRoundFromCurrentMatch()` in `history.ts` keeps the in-memory history store aligned without exposing the writable directly to `matchState`
- Tests: 8 new vitest cases in `matchState.test.ts` covering null on empty, pop+return, lastRoundPoints subtract, defensive floor at 0, atomic matchState update, history-mock invocation, localStorage persistence, multi-undo cascade back to game start, and idempotency (returns null after exhausted)
- Fix (mobile): pinch-zoom on the big score in `TeamCard` was broken since v2.5.27. Switching `touch-action` from `none` → `manipulation` fixed the spurious touchcancel but also let the browser claim 2-finger gestures, swallowing the events our `pinchZoom` action expected. Now uses `pan-y` (strict subset of body's `pan-y pinch-zoom`) — no contradiction with parent so no touchcancel, and pinch is excluded so JS receives the touchmove events and resizes the score font as before
- Refactor: `TeamCard` now derives `displayName` from tournament context instead of `team.name` when in tournament mode. Pairs with the v2.5.32 `handleSwitchSides` regression guard — the visible name now stays in sync with `currentUserSide` even if the parent's `switchSides()` is omitted, eliminating the asymmetry where avatars (context) flipped but the name (store) did not. Test cases added in `TeamCard.test.ts`
